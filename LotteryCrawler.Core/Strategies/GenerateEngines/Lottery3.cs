...
{
    internal class Lottery3 : IGenerateEngine
    {
        public Lottery3()
        {
        }

        public string? Identification => "Lottery3";
        ...
        public BetNumber[] GenerateBet(int[][] history, Card options, short maxNumOfElements)
        {
            ...
            while (successfulNumbers == false && maxGenerations > 0)
            {
                ...
                if (finalBid.Sum(x => options.PresenceElements[x]) <= presenceThreshold
                    && IsStatisticallyProbable(finalBid)
                    && ValidOnBoundaries(finalBid))
        ...
        private bool IsStatisticallyProbable(int[] bid)
        {
            var average = bid.Average();
            return average >= 160 && average <= 200;
        }

        private bool ValidOnBoundaries(int[] bid)
        {
            var average = bid.Sum();
            return average >= 66 && average <= 332;
        }
    }
}
